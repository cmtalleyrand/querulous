...
Entry is decomposed into three additive terms:

1. **Between-voice motion term** (scored once per simultaneity)

| Motion Type | Impact |
|-------------|--------|
| Oblique | +0.5 |
| Contrary | +0.5 |
| Similar (at least one step) | -0.5 |
| Similar (same interval type) | -1.0 |
| Parallel | -1.5 |

`fromRest` modifier: for similar/parallel classes, penalties are multiplied by `0.5`.

2. **Per-voice entry-interval term**

No direct numeric penalty is applied in `scoreEntry` for the size of each voice's entry interval; entry intervals are recorded and evaluated by the exit-resolution penalty function.

3. **Metric term**

| Condition | Impact |
|-----------|--------|
| Strong beat entry | -0.5 |

### Exit Scoring (D → ?)

Exit is decomposed into base resolution quality + optional rest-resolution penalties + per-voice leap-resolution penalties.

1. **Base resolution quality**
...
| To another dissonance (short note + off-beat) | -0.375 |

2. **Rest-resolution penalties**

| Condition | Impact |
|-----------|--------|
| Resolved by abandonment | -0.5 |
| Delayed resolution (one-voice long rest) | -0.3 |
| Invalid two-voice long-rest resolution branch | additional penalty in branch |

3. **Per-voice leap-resolution penalties**

Per-voice penalties are generated by `calculateResolutionPenalty(entryInterval, exitInterval, exitDirection, entryDirection, inSequence)` and added to exit scoring.
...
#### Leap entry (perfect leap / larger)

| Exit Type | Penalty |
|-----------|---------|
| Step or static | 0 |
| Opposite skip | -0.25 |
| Opposite P4/P5 or same-direction skip | -0.75 |
| Other | -1.0 |

Sequence mitigation is intentionally not specified as a standalone policy term in this section; mitigation behavior is handled within the passing-character post-processing architecture.

### Pattern Recognition Bonuses

Patterns are matched in `checkPatterns(prevSim, currSim, nextSim, entryInfo, exitInfo, ctx)`. Each matched pattern emits structured `{type, entryBonus, exitBonus, bonus}` payloads.

#### Strict pattern definitions

1. **Suspension**
   - Entry motion is oblique.
   - The held voice resolves by **step down**.
   - Strong beat: entry `+0.75`, exit `+0.5` (step) + `+0.25` (downward) = total `+1.5`.
   - Weak beat: entry `+0.4`, exit `+0.2` (step) + `+0.1` (downward) = total `+0.7`.

2. **Retardation**
   - Entry motion is oblique.
   - The held voice resolves by **step up**.
   - Strong beat: entry `+0.75`, exit `+0.5`, total `+1.25`.
   - Weak beat: entry `+0.4`, exit `+0.2`, total `+0.6`.

3. **Appoggiatura** (strong-beat gated)
   - A voice enters the dissonance by skip/perfect leap/large leap.
   - The same voice exits by step.
   - Normal case: entry `+2.0`, exit `+0.5`, total `+2.5`.
   - Opposite-step exit case: bonus factor reduced: entry `+1.0`, exit `0`, total `+1.0`.

4. **Cambiata (proper)**
   - Weak beat.
   - Entry by descending step; exit by descending skip of a 3rd.
   - Entry `+0.3`, exit `+1.2`, total `+1.5`.

5. **Cambiata (inverted, weak-beat)**
   - Weak beat.
   - Entry by ascending step; exit by ascending skip of a 3rd.
   - Entry `+0.2`, exit `+0.8`, total `+1.0`.

6. **Cambiata (strong-beat variants)**
   - Same entry/exit profile as above but on strong beat.
   - Descending strong-beat cambiata: entry `+0.1`, exit `+0.4`, total `+0.5`.
   - Inverted strong-beat cambiata: entry `+0.1`, exit `+0.4`, total `+0.5`.

7. **Escape tone**
   - Entry by step.
   - Exit by skip or perfect leap in opposite direction.
   - Entry `0`, exit `+0.5`, total `+0.5`.

8. **Passing tone**
   - Entry by step.
   - Exit by step in same direction.
   - Base bonus `0`; accented (strong-beat) variant adds entry `+0.25`.

9. **Neighbor tone**
   - Entry by step.
   - Exit by step in opposite direction.
   - Base bonus `0`; accented (strong-beat) variant adds entry `+0.25`.

10. **Anticipation**
   - No explicit active bonus branch in current `checkPatterns`; legacy documentation entry retained only for historical terminology reference.
